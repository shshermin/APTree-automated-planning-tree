...
TCP_OFFSET   = [0.0, 0.0, 0.2, 0.0, 0.0, 0.0]  # [m, m, m, rad, rad, rad] wrt flange
PAYLOAD_KG   = 1.0
COG_AT_TCP   = [0.0, 0.0, 0.05]                # [m, m, m]

# ---------------- Targets (EXACTLY as in your working script) -------------

def mmrad_to_mrad(p):
    x,y,z,rx,ry,rz = p
    return [x/1000.0, y/1000.0, z/1000.0, rx, ry, rz]  # keep rads as-is

pt_1 = [-78.03, 589.6, 668.7, 0.575, -3.345, 0.182]
pt_2 = [-193.03, 305.6, 692.15, 0.150, 3.033, 0.007]
pt_3 = [-193.0,  594.6, 200.0, 0.102, -3.254, 0.023]

P1 = mmrad_to_mrad(pt_1)
P2 = mmrad_to_mrad(pt_2)
P3 = mmrad_to_mrad(pt_3)
...
def main():
    ...
    rtde_c.moveL(P1, SPEED, ACC)
    print("[Cmd] P2 (m,rad):", P2)
    rtde_c.moveL(P2, SPEED, ACC)
    print("[Cmd] P3 (m,rad):", P3)
    rtde_c.moveL(P3, SPEED, ACC) 
