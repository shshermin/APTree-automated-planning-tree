...
app.MapPost("/api/aptree/validate", async (AptreeValidateRequest request, IWebHostEnvironment env, CancellationToken ct) =>
{
    ...
    var montiCoreDir = Path.Combine(env.ContentRootPath, "MontiCoreTool");
...
static string ToDisplayName(string rawName)
{
    ...
    var name = Regex.Replace(rawName, "^(BTDecorator_|BTService_|BTServicePlanner_|BTFlowNode_|BTFLowNode_|Call)", "");
    name = name.Replace('_', ' ');
